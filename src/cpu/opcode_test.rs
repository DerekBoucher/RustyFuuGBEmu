...
fn _0xe5() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            ...
            let memory = mock::Memory::new(vec![Opcode::PushHL_0xE5.into(), 0x00, 0x00]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            ...
            cpu.sp = 0x0001;
            cpu.hl.set_word(0x1FFF);
            let memory = mock::Memory::new(vec![Opcode::PushHL_0xE5.into(), 0xFF, 0x1F]);
            return (cpu, memory);
        },
        expected_cycles: 16,
        disable_pc_check: false,
...
fn _0xe6() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.af.hi = 0xFF;
            let memory = mock::Memory::new(vec![Opcode::And8ImmIntoA_0xE6.into(), 0x1F]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
...
fn _0xe7() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0004;
            let memory = mock::Memory::new(vec![Opcode::Reset20h_0xE7.into(), 0x00, 0x00, 0x00]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0002;
            cpu.pc = 0x0020;
            let memory = mock::Memory::new(vec![Opcode::Reset20h_0xE7.into(), 0x00, 0x01, 0x00]);
            return (cpu, memory);
        },
        expected_cycles: 16,
        disable_pc_check: false,
    ...
    let test_cases: Vec<TestCase> = vec![
        ...
        TestCase {
            initial_state: || -> (LR35902, mock::Memory) {
                let mut cpu = LR35902::new();
                cpu.sp = 0x0000;
                let memory = mock::Memory::new(vec![Opcode::AddSigned8ImmIntoSP_0xE8.into(), 0x0F]);
                return (cpu, memory);
            },
            expected_state: || -> (LR35902, mock::Memory) {
                let mut cpu = LR35902::new();
...
fn _0xe9() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.hl.set_word(0xFFFF);
            let memory = mock::Memory::new(vec![Opcode::JumpMemoryHL_0xE9.into()]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
...
fn _0xea() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            ...
            mem_data[0] = Opcode::WriteAInto16ImmAddress_0xEA.into();
            mem_data[1] = 0x1F;
            mem_data[2] = 0xFF;
            cpu.af.hi = 0xFF;
...
fn _0xee() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.af.hi = 0xF0;
            let memory = mock::Memory::new(vec![Opcode::Xor8ImmIntoA_0xEE.into(), 0x0F]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
...
fn _0xef() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0004;
            let memory = mock::Memory::new(vec![Opcode::Reset28h_0xEF.into(), 0x00, 0x00, 0x00]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0002;
            cpu.pc = 0x0028;
            let memory = mock::Memory::new(vec![Opcode::Reset28h_0xEF.into(), 0x00, 0x01, 0x00]);
            return (cpu, memory);
        },
        expected_cycles: 16,
        disable_pc_check: false,
...
fn _0xf0() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let cpu = LR35902::new();
            let mut bytes: Vec<u8> = vec![0x00; 0x10000];
            bytes[0] = Opcode::LoadHiMemOffsetIntoA_0xF0.into();
            bytes[1] = 0xFF;
            bytes[0xFFFF] = 0xFF;
            let memory = mock::Memory::new(bytes);
            return (cpu, memory);
...
fn _0xf2() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            ...
            cpu.bc.lo = 0xFF;
            let mut bytes: Vec<u8> = vec![0x00; 0x10000];
            bytes[0] = Opcode::LoadMemOffsetCIntoA_0xF2.into();
            bytes[0xFFFF] = 0xFF;
            let memory = mock::Memory::new(bytes);
            return (cpu, memory);
        },
...
fn _0xf5() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0003;
            cpu.af.set_word(0x1FFF);
            let memory = mock::Memory::new(vec![Opcode::PushAF_0xF5.into(), 0x00, 0x00]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            ...
            cpu.sp = 0x0001;
            cpu.af.set_word(0x1FFF);
            let memory = mock::Memory::new(vec![Opcode::PushAF_0xF5.into(), 0xFF, 0x1F]);
            return (cpu, memory);
        },
        expected_cycles: 16,
        disable_pc_check: false,
...
fn _0xf6() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.af.hi = 0x00;
            let memory = mock::Memory::new(vec![Opcode::Or8ImmIntoA_0xF6.into(), 0x1F]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
    ...
    let test_cases: Vec<TestCase> = vec![
        ...
        TestCase {
            initial_state: || -> (LR35902, mock::Memory) {
                ...
                cpu.sp = 0x000F;
                let memory =
                    mock::Memory::new(vec![Opcode::LoadSPSigned8ImmIntoHL_0xF8.into(), 0x0F]);
                return (cpu, memory);
            },
            expected_state: || -> (LR35902, mock::Memory) {
                let mut cpu = LR35902::new();
...
fn _0xff() {
    let test_cases: Vec<TestCase> = vec![TestCase {
        initial_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0004;
            let memory = mock::Memory::new(vec![Opcode::Reset38h_0xFF.into(), 0x00, 0x00, 0x00]);
            return (cpu, memory);
        },
        expected_state: || -> (LR35902, mock::Memory) {
            let mut cpu = LR35902::new();
            cpu.sp = 0x0002;
            cpu.pc = 0x0038;
            let memory = mock::Memory::new(vec![Opcode::Reset38h_0xFF.into(), 0x00, 0x01, 0x00]);
            return (cpu, memory);
        },
        expected_cycles: 16,
        disable_pc_check: false,
